...
class LabelManager:
    ...
    def draw_label(self, canvas, label, style, draw_line=False):
        """Draw a label with compact spacing"""
        ...
        # More compact text wrapping
        wrapped_lines = _wrap_text(clean_text, self.field_width - 20, style.font_size)
        
        # Tighter line height
        line_height = style.font_size + 1  # Reduced from +2
        ...
        if draw_line and lines_drawn > 0:
            line_y = current_line_y + line_height + 2
            canvas.setStrokeColor(self.colors['accent'])
            canvas.setLineWidth(1)  # Thinner line
            canvas.line(self.margin_x, line_y, self.margin_x + self.field_width, line_y)
